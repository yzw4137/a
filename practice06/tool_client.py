...
def fetch_webpage(url):
    try:
        ...
        # 使用 urllib.request 打开URL，与 practice05_temp 中的 curl 函数保持一致
...
def main():
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "加载技能的正文内容，当需要使用某个技能时使用",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "skill_name": {
    ...
    # 系统提示词
    system_prompt = f"""你是一个具有文件操作、网络访问、聊天历史搜索、文档仓库访问和技能使用能力的AI助手，能够执行以下操作：
...
9. 使用技能：当用户需要使用某个技能时，使用 load_skill_content 工具加载技能内容，然后按照技能要求执行
...
当用户需要撰写通知、修改通知、润色通知时，请使用 notice 技能。

当用户查询天气时，请使用 fetch_webpage 工具访问 wttr.in 网站，格式为：https://wttr.in/{{城市名}}?lang=zh&format=3
例如：查询青城山天气：https://wttr.in/青城山?lang=zh&format=3
...
当使用 load_skill_content 工具加载技能内容后，你应该按照技能要求执行任务。"""
    ...
    print("=======================")
    print("可用工具：list_files, rename_file, delete_file, create_file, read_file, fetch_webpage, search_chat_history, anythingllm_query, load_skill_content")
    print()
    print("示例：")
    print("1. 列出目录内容：列出当前目录的文件")
    print("2. 访问网页：访问 https://www.example.com 并总结内容")
    print("3. 搜索聊天历史：/search 我之前说了什么")
    print("4. 使用技能：撰写一个五一节放假的通知")
    print()
