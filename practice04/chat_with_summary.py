...
from tools import list_files, rename_file, delete_file, create_file, read_file, curl, anythingllm_query, tools
...
# 系统提示词
system_prompt = """你是一个智能助手，能够使用工具来完成各种文件操作任务和查询文档仓库。
...
6. curl: 通过 curl 访问网页并返回网页内容
   参数：url (要访问的网页 URL)

7. anythingllm_query: 通过AnythingLLM的API查询文档仓库中的信息
   参数：message (查询消息)
...
当用户提到"文档仓库"、"文件仓库"、"仓库"时，使用anythingllm_query工具来查询相关信息。但注意：
- 如果用户只是简单提到"仓库"，应该先询问用户具体想查询什么内容
- 只有当用户明确表达了想查询的内容时，才调用anythingllm_query工具
- 调用时，将用户想查询的具体内容作为message参数

例如：
- 用户问"仓库里有什么？"→ 先询问用户想查询什么具体内容
- 用户问"帮我查一下仓库里关于项目规划的文档"→ 调用anythingllm_query工具，message为"项目规划的文档"
    ...
    try:
        while True:
            ...
            while result.get("tool_called", False):
                ...
                result_msg = f"工具执行完成，结果是：{json.dumps(tool_result.get('result', {}), ensure_ascii=False)}"
                result = send_message(result_msg)
